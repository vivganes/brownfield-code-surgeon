...
async function resetRepo(): Promise<void> {
  ...
  // wipe surgery + plan dirs from a previous toy run
  await fsp.rm(path.join(REPO_ROOT, ARTIFACT_PATHS.surgeryDir), { recursive: true, force: true });
  await fsp.rm(path.join(REPO_ROOT, "plan"), { recursive: true, force: true });
  await ensureSurgeryDir(REPO_ROOT);
  // truncate events file so the UI's replay starts clean
